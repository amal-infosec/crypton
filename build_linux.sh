...
if [ -f /etc/arch-release ]; then
    ...
    sudo pacman -Sy --needed --noconfirm clang cmake ninja pkg-config gtk3 xz libsecret jsoncpp zip alsa-lib mpv
elif [ -f /etc/debian_version ]; then
    echo "📦 Detected Debian/Ubuntu. Installing dependencies..."
    sudo apt-get update
    sudo apt-get install -y clang cmake ninja-build pkg-config libgtk-3-dev liblzma-dev libsecret-1-dev libjsoncpp-dev zip libasound2-dev libmpv-dev
else
